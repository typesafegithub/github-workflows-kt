...
public suspend fun ActionCoords.generateBinding(
    ...
    val typingsResolved = typings ?: this.provideTypes(metadataRevision, httpClient = httpClient)
        ...
        typingsResolved.source?.let {
            val actionBindingSourceCode =
                generateActionBindingSourceCode(
                    ...
                    className = className,
                    deprecationMessage =
                        typingsResolved.takeIf { it.fromFallbackVersion }?.let {
...
private fun generateActionClass(
    ...
    TypeSpec
        ...
        .markDeprecated(bindingVersion, deprecationMessage, replaceWith)
                ...
                listOfNotNull(
                    outputClassName?.let {
                        PropertySpec
                            ...
                            .addKdocIfNotEmpty(
                                value.description.escapedForComments.removeTrailingWhitespacesForEachLine(),
                            ).build()
                    },
                    PropertySpec
                        .builder(
...
private fun TypeSpec.Builder.deprecateBindingVersion(bindingVersion: BindingVersion): TypeSpec.Builder {
    if (bindingVersion.isDeprecated) {
        addAnnotation(
            AnnotationSpec
                .builder(Deprecated::class.asClassName())
                .addMember("%S", "Use a non-deprecated binding version")
                .build(),
        )
    }
    return this
...
private fun TypeSpec.Builder.markDeprecated(
    ...
    if (!bindingVersion.isDeprecated && ((deprecationMessage != null) || (replaceWith != null))) {
        addAnnotation(
            AnnotationSpec
                .builder(Deprecated::class.asClassName())
    ...
    inputs
        ...
        .forEach { (key, input) ->
            ...
            if (input.shouldBeRequiredInBinding()) {
                beginControlFlow(
                    "require(${inputProperties.joinToString(" || ") { "($it != null)" }})",
                    propertyBaseName,
                )
                addStatement(
                    "%S",
